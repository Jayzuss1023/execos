...
import { ParsedEmail } from "./google/gmail";
...
const emailAnalysisSchema = z.object({
  ...
  actionItems: z
    ...
    .describe("Concrete action items extraction from the email"),
  ...
  calendarEvents: z
    .array(
      z.object({
        title: z
          .string()
          .describe(
            "Short event title, e.g., 'Review proposal' or 'Meerting with Sarah'",
          ),
        description: z
          .string()
    ...
    .describe(
      "Calendar events to create from this evmail. Create events for deadlines, meetings, reminders, or any time-sensitive items mentioned . If someone says 'by Friday', create a reminder event on that day.",
    ),
  category: z
...
export async function analyzeWithAI(
  ...
  if (upcomingEvents.length > 0) {
    const eventsList = upcomingEvents
      .map(
        (e) =>
          `- ${e.summary} (${e.start} - ${e.end}${e.location ? `, at ${e.location}` : ""})`,
      )
      .join("\n");
  ...
  const result = await generateText({
    ...
    prompt: `
    You are an AI assistant analyzing emails. Today's date is ${today}
    ...
    - If the email mentions ANY deadline, meeting, or time-sensitive event (e.g., "by Friday", "next Thursday", "schedule a call"), you MUST create a calendar event for it. Convert relative dates like "Friday" so actualy ISO dates based on today's date (${today}).
